...
* Version      : 7.20
* Description  : RZ/V2L DRP-AI Sample Application for PyTorch ResNet Coral Camera version
***********************************************************************************************************************/
...
/*Global Variables*/
static float drpai_output_buf[NUM_CLASSES];
static float bg_embedding[NUM_CLASSES] = {0};
static uint32_t capture_address;
static uint64_t udmabuf_address = 0;
static Image img;
static int bg_frame = 5;
...
int8_t calibrate_background(float* floatarr)
{
    ...
    printf("Background calibration is done\n");
    return 0;                                            
}
...
int8_t print_result(float* floatarr, Image* img)
{
    ...
    if((floatarr[0] < floatarr[1]) && (!is_bg))
        img->write_string_rgb("Face Spoofing Detected.!!!", CAM_IMAGE_WIDTH * RESIZE_SCALE + TEXT_WIDTH_OFFSET, LINE_HEIGHT+LINE_HEIGHT_OFFSET+LINE_HEIGHT, CHAR_SCALE_SMALL, WHITE_DATA);
...
void *R_Capture_Thread(void *threadid)
{
    ...
    while (1)
    {
        ...
        /* Capture Coral camera image and stop updating the capture buffer */
...
int8_t R_Main_Process()
{
    ...
    while (1)
    {
        ...
        if (img_obj_ready.load())
        {
            ...
            img.draw_rect(OUTPUT_LEFT, OUTPUT_TOP, CROPPED_IMAGE_WIDTH-1, CROPPED_IMAGE_HEIGHT-1, YELLOW_DATA);
